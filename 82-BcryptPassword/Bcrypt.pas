// Based on
// https://github.com/PonyPC/BCrypt-for-delphi-lazarus-fpc
// https://stackoverflow.com/questions/32854236/multi-device-bcrypt-library-for-delphi-xe8

unit BCrypt;

//for testing please use https://www.browserling.com/tools/bcrypt
// make sure you use the right rounds number
// "rounds" could be equal to 13

interface

uses
  Classes, SysUtils, RegularExpressions, Types;

type
  TRoundsCount = 4..30;

function CheckPassword(const Str: string; const Hash: string; const rounds: TRoundsCount = 10): boolean;
function HashPassword(const Str: string; const salt: string; const rounds: TRoundsCount = 10): string; overload;
function HashPassword(const Str: string; const rounds: TRoundsCount = 10): string; overload;
...
function BsdBase64Encode(const Data: TBytes; len: integer): string;
var
  i: integer;
  b1, b2: integer;
begin
...
function BsdBase64Decode(const s: string): TBytes;
...
function CryptRaw(const key, salt: TBytes; const rounds: integer): TBytes;
var
...
function FormatPasswordHashForBsd(const salt, hash: TBytes; const rounds: TRoundsCount): string;
var
  saltString: string;
  hashString: string;
begin
  ...
  Result := Format('$2a$'+IntToStr(rounds)+'$%s%s', [saltString, hashString]);
end;

function HashPassword(const Str: string; const salt: string; const rounds: TRoundsCount = 10): string;
var
  password: string;
  key, saltBytes, Hash: TBytes;
  i: integer;
begin
  password := String(AnsiToUtf8(str));
  ...
  Hash := CryptRaw(key, saltBytes, rounds);
  Result := FormatPasswordHashForBsd(saltBytes, Hash, rounds);
end;
...
function HashPassword(const Str: string; const rounds: TRoundsCount = 10): string; overload;
begin
  Result := HashPassword(Str, GenerateSalt, rounds);
end;
...
var
  ...
  match   : TMatch;
begin
  RegexObj := TRegEx.Create('^\$2a\$'+IntToStr(rounds)+'\$([\./0-9A-Za-z]{22})', [roIgnoreCase]);
  match := RegexObj.Match(Hash);
  Result := match.Success and (HashPassword(Str, Copy(match.Value, 8, 22), rounds) = Hash);
end;
