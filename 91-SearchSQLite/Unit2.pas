...
type
  ...
  public
    { Public declarations }
    procedure SearchDatabase(const ASearchString: String);
    procedure InitializeDatabase;
    procedure DelayedSetFocus(Control: TControl);
  end;
  const
    DB_FILENAME = 'search_sqlite.sdb';
...
procedure TDataModule1.SearchDatabase(const ASearchString: string);
begin
  FDQuerySearch.Close;

  if ASearchString<>'' then
    begin
      FDQuerySearch.MacroByName('Where').AsRaw := 'name LIKE :SearchString';
      FDQuerySearch.ParamByName('SearchString').AsString := '%'+ASearchString+'%';
    end
  else
    begin
      FDQuerySearch.MacroByName('Where').AsRaw := '';
      //FDQuerySearch.ParamByName('SearchString').AsString := '';
    end;

  FDQuerySearch.Open;
end;

procedure TDataModule1.DelayedSetFocus(Control: TControl);
begin
